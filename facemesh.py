...
import os
import numpy as np
from mesh.mesh import Mesh, MeshViewer, MeshViewers
import time
...
class FaceData(object):
    ...
    def vec2meshTrimesh(self, vec, col = None):
        import trimesh as tri
        vec = vec.reshape((self.n_vertex, 3)) * self.std + self.mean
        if col is None:
            return tri.Trimesh(vertices=vec, faces=self.reference_mesh.f)
        else:
            return tri.Trimesh(vertices=vec, faces=self.reference_mesh.f, vertex_colors=col)

    def vec2meshTrimesh2(self, vec, col=None):
        import trimesh as tri
